...
void ecb_crypt(const uint8_t *in, const uint8_t *key,
               uint8_t *out, int enc)
{
    CipherCtx c;

    if (EVP_CipherInit_ex(c.ctx, EVP_aes_128_ecb(), nullptr,
                          key, nullptr, enc) != 1)
...
void cbc_crypt(uint8_t *out, const uint8_t *in, uint32_t length,
               const uint8_t *key, const uint8_t *iv, int enc)
{
    ...
    if (EVP_CipherInit_ex(c.ctx, EVP_aes_128_cbc(), nullptr,
                          key, iv, enc) != 1)
    ...
    if (enc == 1)
    {
        ...
        if (full_blocks > 0)
        {
            if (EVP_CipherUpdate(c.ctx, out, &out_len,
                                 in, static_cast<int>(full_blocks * AES128_BLOCK)) != 1)
        ...
        if (remainder_bytes > 0)
        {
            ...
            int partial_len = 0;
            if (EVP_CipherUpdate(c.ctx, out + out_len, &partial_len,
                                 staging, AES128_BLOCK) != 1)
    ...
    else
    {
        ...
        if (EVP_CipherUpdate(c.ctx, out, &out_len,
                             in, static_cast<int>(total_bytes)) != 1)
