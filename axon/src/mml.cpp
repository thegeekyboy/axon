		...
		namespace core {
			...
			bool mml::connect(std::string hostname, unsigned short int port)
			{
				...
				try {
					...
					throw axon::exception(__FILE__, __LINE__, __func__, "Generic: Encountered exception while creating thread!");
				}
			...
			bool mml::conditions(std::string success, std::string error, std::string breaks)
			{
				try {
					...
					throw axon::exception(__FILE__, __LINE__, __func__, "Regex: Error compiling condition expression");
				}
			...
			bool huamsc::login(std::string username, std::string password)
			{
				...
				if (exec("LGI:OP=\"" + _username + "\", PWD=\"" + _password + "\";", &response, 5) != 0)
					throw axon::exception(__FILE__, __LINE__, __func__, "MSC: Login error!");
			...
			{
				if (_msc.size() > 0)
					throw axon::exception(__FILE__, __LINE__, __func__, "MSC: Still registered with " + _msc + ", unregister first.");

				if (exec("LGO:OP=\"" + _username + "\";") != 0)
					throw axon::exception(__FILE__, __LINE__, __func__, "MSC: Logout error");
			...
			bool huamsc::reg(std::string neid)
			{
				...
				if (_msc.size() > 0)
					throw axon::exception(__FILE__, __LINE__, __func__, "MSC: Already registered with " + _msc);

				if (exec("REG NE:NAME=\"" + neid + "\";") != 0)
					throw axon::exception(__FILE__, __LINE__, __func__, "MSC: Error connecting to Network Element");
			...
			{
				if (_msc.size() <= 0)
					throw axon::exception(__FILE__, __LINE__, __func__, "MSC: Not registered with any MSC.");
			...
			bool huamsc::query(telecom::subscriber &subs)
			{
				...
				if (subs.msisdn.size() == 0)
					throw axon::exception(__FILE__, __LINE__, __func__, "HLR: Invalid MSISDN");
			...
			bool huahlr::login(std::string username, std::string password)
			{
				...
				if (exec("LGI: HLRSN=1, OPNAME=\"" + _username + "\", PWD=\"" + _password + "\";") != 1)
					throw axon::exception(__FILE__, __LINE__, __func__, "HLR: Login error");
			...
			{
				if (exec("LGO;") != 1)
					throw axon::exception(__FILE__, __LINE__, __func__, "HLR: Logout error");
			...
			bool huahlr::remove(std::string msisdn)
			{
				if (exec("RMV SUB: ISDN=\"" + msisdn + "\", RMVKI=TRUE;") != 1)
					throw axon::exception(__FILE__, __LINE__, __func__, "HLR: Error removing subscriber");
			...
			bool huahlr::vlrnum(telecom::subscriber &subs)
			{
				...
				if (exec("LST VLRNUM: ISDN=\"" + subs.msisdn + "\";", &response) != 1)
					throw axon::exception(__FILE__, __LINE__, __func__, "HLR: Error performing query on subscriber");
