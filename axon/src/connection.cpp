	...
	namespace transport
	{
		namespace transfer
		{
			connection::connection(std::string hostname, std::string username, std::string password)
			{
				...
				{
					throw axon::exception(__FILE__, __LINE__, __func__, "Wrong hostname format");
					return;
				}

				if (username.size() <= 0)
				{
					throw axon::exception(__FILE__, __LINE__, __func__, "Username cannot be empty");
					return;
				}

				if (!boost::regex_match(username, regex_username))
				{
					throw axon::exception(__FILE__, __LINE__, __func__, "Username format is wrong");
					return;
				}
