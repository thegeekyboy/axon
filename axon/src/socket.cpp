		...
		namespace tcpip
		{
			...
			bool socks::init()
			{
				...
				if ((_fd = socket(PF_INET, SOCK_STREAM, 0)) < 0)
					throw axon::exception(__FILE__, __LINE__, __func__, "Error initializing socket()");

				if (setsockopt(_fd, SOL_SOCKET, SO_REUSEADDR, (void *) &on, sizeof(on)) == -1)
					throw axon::exception(__FILE__, __LINE__, __func__, "Error setting options with setsockopt()");

				if (setsockopt(_fd, SOL_SOCKET, SO_LINGER, (void *) &lng, sizeof(lng)) == -1)
					throw axon::exception(__FILE__, __LINE__, __func__, "Error setting linger with setsockopt()");
			...
			bool socks::open(std::string host, unsigned short port)
			{
				...
				if (inet_aton(_host.c_str(), &sin.sin_addr) == 0)
				{
					if ((phe = gethostbyname(_host.c_str())) == NULL)
						throw axon::exception(__FILE__, __LINE__, __func__, "Cannot resolve hostname");

					memcpy((char *)&sin.sin_addr, phe->h_addr, phe->h_length);
				}

				if (connect(_fd, (struct sockaddr *)&sin, sizeof(sin)) == -1)
					throw axon::exception(__FILE__, __LINE__, __func__, "Socket connection connect() error");

				if (!nonblocking(_fd, true))
					throw axon::exception(__FILE__, __LINE__, __func__, "Could not set non-blocking mode");
			...
			bool socks::writeline(std::string data)
			{
				...
				if (write(data.c_str(), data.size()) <= 0)
					throw axon::exception(__FILE__, __LINE__, __func__, "Error writing()-ing to socket");
